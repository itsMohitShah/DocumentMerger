...
def extract_companyname(pdf_path):
    ...
    """
    try:
        logging.info(f"Extracting company name from PDF: {pdf_path}")
...
def filter_full_names(name_list):
    ...
    """
    try:
        logging.info(f"Filtering full names from the list: {name_list}")
        full_names_only = []
        ...
        logging.info(f"Filtered full names: {full_names_only}")
        return full_names_only
    except Exception as e:
        logging.error(Fore.RED + f"Error filtering full names: {e}" + Style.RESET_ALL)
        return []
...
def extract_name_from_pdf(pdf_path):
    ...
    """
    try:
        logging.info(f"Extracting names from PDF: {pdf_path}")
        # Read the PDF
        ...
        for ent in doc.ents:
            if ent.label_ == "PERSON":
                ...
                if ent_text.lower() in ["mohit shah", "jan erxleben", "nirav doshi"]:
                    logging.info(f"Skipping excluded name: {ent_text}")
                    continue
                list_names.append(ent.text)
        logging.info(f"Extracted raw names: {list_names}")
...
def search_linkedin(list_names, companyname):
    ...
    try:
        ...
        logging.info(f"Starting LinkedIn search for names: {list_names} and company: {companyname}")
        ...
        logging.error(Fore.RED + f"Error during LinkedIn search: {e}" + Style.RESET_ALL)
...
def main_linkedin_search(pdf_path):
    ...
    """
    try:
        logging.info(f"Starting LinkedIn search process for PDF: {pdf_path}")
        most_recent_pdf = find_most_recent_pdf(pdf_path)
        logging.info(f"Most recent PDF found: {most_recent_pdf}")
